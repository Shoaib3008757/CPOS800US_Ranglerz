...
import com.ranglerz.utils.Constant;
...
public class FingerprintAPI {
	...
	public synchronized int PSGetImage() {
		byte[] srcCmd = { (byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff,
				(byte) 0xff, (byte) 0xff, 0x01, 0x00, 0x03, 0x01, 0x00, 0x05 };
		byte[] cmdHead = { (byte) 0xca, (byte) 0xdf, (byte) 0x03, (byte) 0x35 };
		int len = srcCmd.length;
		byte[] dstCmd = new byte[cmdHead.length + 2 + len + 1];
		System.arraycopy(cmdHead, 0, dstCmd, 0, cmdHead.length);
		dstCmd[cmdHead.length] = (byte) 0;
		dstCmd[cmdHead.length + 1] = (byte) len;
		System.arraycopy(srcCmd, 0, dstCmd, cmdHead.length + 2, len);
		dstCmd[dstCmd.length - 1] = (byte) 0xE3;

		SerialPortManager.getInstance().write(dstCmd);
		...
		do {
			...
			if (isFirst) {
				...
				if (size < startLength) {
					...
					length += 12;
					SystemClock.sleep(5);
					SerialPortManager.getInstance().write(repeatCommand);
				} else {
					System.arraycopy(buffer, 0, bufferImage, 0, size);
					length += size;
					SystemClock.sleep(5);
					SerialPortManager.getInstance().write(nextCommand);
				}
				continue;
			}
			Log.i("whw", "size=" + size);
	...
	private void sendCommand(byte[] command) {
		...
		PacketHeader pHeader = new PacketHeader(command, Constant.FINGER_ID);
